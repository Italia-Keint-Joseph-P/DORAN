...
def admin_load_json():
    """
    Load JSON content from a file for the admin JSON editor.
    """
    if not is_admin(current_user):
        return jsonify({'status': 'error', 'message': 'Unauthorized access'})
    ...
    if not filepath.startswith('database/'):
        return jsonify({'status': 'error', 'message': 'Filepath must start with "database/"'})
    ...
    try:
        ...
        return jsonify({'status': 'error', 'message': f'Failed to load JSON: {str(e)}'})

@app.route('/admin/save_json', methods=['POST'])
@login_required
def admin_save_json():
    """
    Save JSON content to a file for the admin JSON editor.
    """
    if not is_admin(current_user):
        return jsonify({'status': 'error', 'message': 'Unauthorized access'})
    ...
    try:
        ...
        return jsonify({'status': 'error', 'message': f'Failed to save JSON: {str(e)}'})

@app.route('/admin/upload_json', methods=['POST'])
@login_required
def admin_upload_json():
    """
    Upload and save a JSON file for the admin JSON editor.
    """
    if not is_admin(current_user):
        return jsonify({'status': 'error', 'message': 'Unauthorized access'})
    ...
    if not filepath.startswith('database/') or not file:
        return jsonify({'status': 'error', 'message': 'Invalid filepath or no file provided'})
